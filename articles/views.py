...
def can_publish(user):
    return user.is_superuser or user_in_group(user, 'Publisher') or user_in_group(user, 'Admin')
...
def article_detail(request, slug):
    ...
    if article.status != NewsArticle.PUBLISHED:
        if not request.user.is_authenticated or not (
            article.author == request.user or can_publish(request.user) or request.user.is_superuser
        ):
...
def article_create(request):
    if not (user_in_group(request.user, 'Editor') or user_in_group(request.user, 'Publisher') or user_in_group(request.user, 'Admin') or request.user.is_superuser):
        raise PermissionDenied
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        article.save()
        message = 'Published' if article.status == NewsArticle.PUBLISHED else 'Sent for review' if article.status == NewsArticle.PENDING else 'Saved as draft'
        messages.success(request, f'Article “{article.title}” {message}.')
...
def pending_articles(request):
    ...
    articles = NewsArticle.objects.filter(status=NewsArticle.PENDING)
